...
// Jei esame spausdinimo režime (su duomenimis), generuojame spausdinamą lentelę
if ($print_mode && !empty($selected_olympiad) && !empty($participants)) {
    ...
    echo '<style>
        @media print {
            .evaluation-title { text-align: center; font-size: 18px; font-weight: bold; margin-bottom: 10px; }
            .protocol-number { text-align: center; font-size: 14px; margin-bottom: 20px; }
            .participant-count { text-align: center; margin-top: 20px; }
            .signature { text-align: center; margin-top: 20px; font-size: 14px; }
        }
    </style>';
    ...
    echo generate_printable_table($selected_olympiad, $institution, $headers, $data, [
        'signature_text' => 'Atsakingo asmens parašas',
        'signature_name' => '',
        'include_back_button' => true,
        'back_button_url' => SITE_URL . '/modules/reports/evaluation_sheets.php'
    ]);
    echo '<div class="participant-count">Dalyvių skaičius: ' . $participant_count . '</div>';
    echo '</div>';
    exit;
}

// Jei esame spausdinimo režime (tuščias protokolas pildymui)
